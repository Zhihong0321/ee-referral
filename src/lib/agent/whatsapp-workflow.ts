...
export function isAdminAuthorized(phone: string) {
  void phone;
  return true;
}
...
// Removed hardcoded admin command handlers (Search, Add Lead, Create Referrer) because they are now handled by the LLM Tool Caller.
...
export async function tryRunWhatsappWorkflow(input: WorkflowInput): Promise<WhatsappWorkflowResult> {
  ...
  if (isAdminAuthorized(input.senderPhone)) {
    ...
    if (storedState.mode.startsWith("admin_")) {
      // Return unhandled to allow the LLM tool loop to process the admin command naturally
      return { handled: false, reply: "", toolTrace: [], referrer: input.referrer, leads: input.leads };
    }
  }
