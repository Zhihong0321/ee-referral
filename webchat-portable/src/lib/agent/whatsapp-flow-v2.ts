...
function buildSystemPrompt(
  ...
  if (isAdmin) {
    lines.push(
      ...
      "8. This agent only does referral-lead work — never reply with a generic open-ended \"How can I help you?\". When the admin's message is just a greeting (hi/hello/hey) with no request, greet them briefly and name what you can do for them here, e.g. \"Hi! Which referrer's leads do you want to check or update?\" Do this especially on the first message of a conversation.",
      "9. Recency rule: this is a human conversation, not a database of equally-valid facts — when two messages describe the same lead/detail differently (a corrected phone number, a changed area, a different preferred agent), the MOST RECENT one is the truth; people correct themselves, they don't hold two facts at once. Only treat an older message as authoritative when the admin explicitly says so (e.g. 'no, use the first one'). This does not override rule 7 — PENDING LEAD DATA (or its absence) still wins over anything in history for a bare 'add this lead'.",
      "",
      "YOUR ADMIN TOOLS:",
  ...
  lines.push(
    ...
    "12. This agent only does referral-lead work — never reply with a generic open-ended \"How can I help you?\". When the user's message is just a greeting (hi/hello/hey/good morning) with no request, greet them briefly and name the two things they can do, e.g. \"Hi! Want to add a new lead, or check your existing leads?\" Do this especially on the first message of a conversation.",
    "",
    `Portal: ${PORTAL_URL}`,
    "",
...
async function callAgentModel(
  ...
  if (format === "openai") {
    ...
    if (!response.ok) {
      throw new Error(`LLM HTTP ${response.status} [endpoint: ${endpoint}, model: ${LLM_MODEL}]: ${text.slice(0, 300)}`);
    }
